                ...
                <tbody>
                    {patients.map((p) => (
                        <tr key={p.id} className="bg-slate-900/50 border-b border-slate-800 hover:bg-slate-800/50 transition">
                            ...
                            <td className="px-6 py-4">{p.faixa === 'Criança' ? p.responsavel : 'N/A'}</td>
                            <td className="px-6 py-4">{p.convenio}</td>
                            <td className="px-6 py-4">{p.carteirinha}</td>
                            <td className="px-6 py-4">{p.tipoAtendimento}</td>
                            <td className="px-6 py-4">
                                <div className="flex flex-wrap gap-1">
                                    {p.profissionais.map(pro => <span key={pro} className="bg-slate-700 text-slate-300 text-xs font-medium px-2 py-0.5 rounded-full">{pro}</span>)}
                                </div>
                            </td>
                            <td className="px-6 py-4">
                                <div className="flex flex-wrap gap-1">
                                    {p.especialidades.map(esp => <span key={esp} className="bg-slate-700 text-slate-300 text-xs font-medium px-2 py-0.5 rounded-full">{esp}</span>)}
                                </div>
                            </td>
                            <td className="px-6 py-4">
